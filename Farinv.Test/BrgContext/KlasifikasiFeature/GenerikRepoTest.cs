using Farinv.Domain.BrgContext.KlasifikasiFeature;
using Farinv.Infrastructure.BrgContext.KlasifikasiFeature;
using FluentAssertions;
using Moq;

namespace Farinv.Test.BrgContext.KlasifikasiFeature;

public class GenerikRepoTests
{
    private readonly Mock<IGenerikDal> _generikDalMock;
    private readonly GenerikRepo _repository;

    public GenerikRepoTests()
    {
        _generikDalMock = new Mock<IGenerikDal>();
        _repository = new GenerikRepo(_generikDalMock.Object);
    }

    [Fact]
    public void UT1_GivenExistingEntity_WhenSaveChanges_ThenUpdateIsCalled()
    {
        // Arrange
        var existingModel = CreateTestModel();
        _generikDalMock
            .Setup(x => x.GetData(It.IsAny<IGenerikKey>()))
            .Returns(CreateTestDto());

        // Act
        _repository.SaveChanges(existingModel);

        // Assert
        _generikDalMock.Verify(x => x.Update(It.IsAny<GenerikDto>()), Times.Once);
        _generikDalMock.Verify(x => x.Insert(It.IsAny<GenerikDto>()), Times.Never);
    }

    [Fact]
    public void UT2_GivenNewEntity_WhenSaveChanges_ThenInsertIsCalled()
    {
        // Arrange
        var newModel = CreateTestModel();
        var key = CreateTestKey();
        _generikDalMock
            .Setup(x => x.GetData(key))
            .Returns((GenerikDto)null!);

        // Act
        _repository.SaveChanges(newModel);

        // Assert
        _generikDalMock.Verify(x => x.Insert(It.IsAny<GenerikDto>()), Times.Once);
        _generikDalMock.Verify(x => x.Update(It.IsAny<GenerikDto>()), Times.Never);
    }

    [Fact]
    public void UT3_GivenExistingEntity_WhenLoadEntity_ThenEntityIsReturned()
    {
        // Arrange
        var expectedDto = CreateTestDto();
        var key = CreateTestKey();
        _generikDalMock
            .Setup(x => x.GetData(key))
            .Returns(expectedDto);

        // Act
        var result = _repository.LoadEntity(key);

        // Assert
        result.HasValue.Should().BeTrue();
        result.Match(
            onSome: model => model.Should().NotBeNull(),
            onNone: () => Assert.Fail("Expected Some but got None"));
    }

    [Fact]
    public void UT4_GivenNonExistingEntity_WhenLoadEntity_ThenNoneIsReturned()
    {
        // Arrange
        var key = CreateTestKey();
        _generikDalMock
            .Setup(x => x.GetData(key))
            .Returns((GenerikDto)null!);

        // Act
        var result = _repository.LoadEntity(key);

        // Assert
        result.HasValue.Should().BeFalse();
        result.Match(
            onSome: model => Assert.Fail("Expected None but got Some"),
            onNone: () => { });
    }

    [Fact]
    public void UT5_GivenKey_WhenDeleteEntity_ThenDeleteIsCalled()
    {
        // Arrange
        var key = CreateTestKey();

        // Act
        _repository.DeleteEntity(key);

        // Assert
        _generikDalMock.Verify(x => x.Delete(key), Times.Once);
    }

    [Fact]
    public void UT6_GivenEmptyList_WhenListData_ThenEmptyListIsReturned()
    {
        // Arrange
        _generikDalMock
            .Setup(x => x.ListData())
            .Returns((IEnumerable<GenerikDto>)null!);

        // Act
        var result = _repository.ListData();

        // Assert
        var generikTypes = result.ToList();
        generikTypes.Should().NotBeNull();
        generikTypes.Should().BeEmpty();
    }

    [Fact]
    public void UT7_GivenListWithItems_WhenListData_ThenListWithModelsIsReturned()
    {
        // Arrange
        var dtos = new List<GenerikDto> { CreateTestDto(), CreateTestDto() };
        _generikDalMock
            .Setup(x => x.ListData())
            .Returns(dtos);

        // Act
        var result = _repository.ListData();

        // Assert
        var generikTypes = result.ToList();
        generikTypes.Should().NotBeNull();
        generikTypes.Count.Should().Be(2);
    }

    private static GenerikType CreateTestModel()
        => GenerikType.Default;

    private static GenerikDto CreateTestDto()
        => GenerikDto.FromModel(GenerikType.Default); 

    private static IGenerikKey CreateTestKey()
        => GenerikType.Key("A"); 
}